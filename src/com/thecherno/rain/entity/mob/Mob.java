...
public abstract class Mob extends Entity {
    ...
    protected Sprite sprite; // protected means this variable can only be used within this class and all of
                             // its sub classes.
    // if you had everything you programmed in public, the game would run the same
    // but it's a good programming practice,
    // helps everything be more organized and minimizes errors like accessing the
    // wrong variable
    ...
    private boolean collision(int xLocation, int yLocation) {
        ...
        for (int corner = 0; corner < 4; corner++) { // checking for each corner of a tile
            int xTilePixel = ((x + xLocation) + corner % 2 * 12 - 6) / 16; // modulus 2, checking for x coordinates.
                                                                           // width is 2 because 2 dots in the square
            int yTilePixel = ((y + yLocation) + corner / 2 * 12 + 3) / 16; // divide by 2 for y/ *12+3 adjusts where the
                                                                           // center point is of the solid tile
            // if any of the corners belongs to a solid tile, don't move through it
            if (level.getTile(xTilePixel, yTilePixel).solid())
                solid = true; // rough collision detection
        }
        return solid;
    }
